...
def objective(trial):
    ...
    num_layers = trial.suggest_int('num_layers', 1, 3)  # Reduced max layers for DNN
    dropout_rate = trial.suggest_float('dropout_rate', 0.2, 0.5)
    lr = trial.suggest_float('lr', 1e-4, 1e-2, log=True)
    batch_size = trial.suggest_categorical('batch_size', [128, 256, 512])

    # Prepare data (no need for unsqueeze as DNN takes flat input)
...
auc_roc_scores, auc_pr_scores, _, _ = evaluate_model(model, test_loader, num_classes=len(label_encoder.classes_),
                                                     compute_pr=True)
