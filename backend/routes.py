...
def upload_pcap():
    ...
    print("cicflowmeter_dir:", cicflowmeter_dir)
    ...
    #         os.remove(path)
    print(f"Đã lưu PCAP tại: {pcap_save_path}")
    print(f"Đã lưu CSV tại: {csv_save_path}")
    print(f"Đã lưu CSV refactored tại: {refactored_csv_path}")
    results = [{'log_index': i, 'risk_level': label} for i, label in enumerate(risk_labels)]
    return jsonify({'upload_id': upload.id, 'results': results})
