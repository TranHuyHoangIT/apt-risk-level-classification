...
export const startRealtimeMonitoring = async (file) => {
    try {
        const formData = new FormData();
        formData.append('file', file);
        const res = await api.post('/start-realtime', formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
        });
        return res.data;
    } catch (error) {
        return {
            error: error.response?.data?.error || 'Đã có lỗi khi bắt đầu giám sát thời gian thực',
            status: error.response?.status,
        };
    }
};
...
export const stopRealtimeMonitoring = async (sessionId) => {
    try {
        const res = await api.post(`/stop-realtime/${sessionId}`);
        return res.data;
    } catch (error) {
        return {
            error: error.response?.data?.error || 'Đã có lỗi khi dừng giám sát thời gian thực',
            status: error.response?.status,
        };
    }
};